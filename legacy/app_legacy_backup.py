...
from src.parsers.ofx import OfxParser
...
# --- CONFIG ---
st.set_page_config(page_title="Auditor Contábil", layout="wide")

st.title("🕵️ Auditor Contábil - Reconciliação Bancária")
...
def select_folder_callback():
    ...
    if selected_folder:
        ...
        # We DO NOT scan here anymore to avoid blocking without feedback
        # The main loop will handle it.

# --- SIDEBAR: INPUTS ---
st.sidebar.header("1. Livro Diário")
ledger_file = st.sidebar.file_uploader("Upload do PDF ou CSV", type=["pdf", "csv"])

st.sidebar.header("2. Extratos Bancários")
input_method = st.sidebar.radio("Fonte dos Arquivos:", ["Upload Manual", "Escanear Pasta"])

bank_files = []
selected_files_df = pd.DataFrame()

if input_method == "Upload Manual":
    uploaded_files = st.sidebar.file_uploader("Arquivos (PDF/OFX)", type=["pdf", "ofx"], accept_multiple_files=True)
    if uploaded_files:
        # Wrap uploaded files in a format compatible with our loop
    ...
    # Auto-Scan Logic
    # Trigger if path is set AND (path changed OR no results yet)
    should_scan = False
    ...
    if not ledger_file:
        ...
        with st.spinner("Processando arquivos..."):
            ...
            try:
                ...
                # Ensure amount is float for consistency with bank data (which uses pd.to_numeric)
            ...
            for i, file_info in enumerate(bank_files):
                ...
                try:
                    # Use Factory for everything now
                    parser = None
                    input_arg = None
                    
                    if isinstance(fobj, str):
                        parser = ParserFactory.get_parser(fobj)
                        input_arg = fobj
                    else:
                        # UploadedFile handling
                        if fname.lower().endswith('.ofx'):
                             parser = OfxParser()
                             input_arg = fobj # OFX parser handles file-like object usually or we adapt
                             # Actually OFX parser expects file path in current implementation?
                             # Checked OfxParser: yes, usually parses file path or file object. 
                             # Let's save safely to temp.
                             import tempfile
            ...
            if all_bank_dfs:
                # Use Consolidator to merge and deduplicate
                df_bank = TransactionConsolidator.consolidate(all_bank_dfs)
                
                # Filter Zero Amounts (Saldos/Markers)
                initial_count = len(df_bank)
                df_bank = df_bank[abs(df_bank['amount']) > 0.009].copy()
                removed_zeros = initial_count - len(df_bank)
                
                st.success(f"✅ Extratos processados: {len(df_bank)} transações de {len(bank_files)} arquivos (após deduplicação). {removed_zeros} itens de valor zero removidos.")
            else:
                st.error("❌ Nenhuma transação bancária extraída.")
                st.stop()
...
if st.session_state.get('data_processed', False):
    ...
    unmatched_l = results['unmatched_l']
    unmatched_b = results['unmatched_b']
    ...
    diff = abs(unmatched_l['amount'].sum() - unmatched_b['amount'].sum())
    ...
    with col_f1:
        ...
        default_options = ['Pendente - Banco', 'Pendente - Diário'] # Default to showing problems
    ...
    pdf_exporter = PDFReportExporter(
        company_name="1266 - MCM FOODS LTDA", # Could extract from UI/CSV filename if dynamic needed
